#!/bin/bash
# Goblin Inspector - HTTP Transport
# Connects MCP Inspector to Goblin via HTTP transport
#
# Usage: ./goblin-inspector-http.sh [--port <port>] [--url <url>]
#
...
check_goblin_http() {
    if curl -s --connect-timeout 5 "$GOBLIN_URL/health" > /dev/null 2>&1; then
        echo_success "Goblin is responding at: $GOBLIN_URL"
        return 0
    else
        echo_error "Goblin is not responding at: $GOBLIN_URL"
        echo_info "Make sure Goblin is running in HTTP mode"
        echo_info "Example: goblin start --port 3000"
        return 1
    fi
}
...
Connects MCP Inspector to Goblin via HTTP transport for interactive debugging.
...
main() {
    parse_args "$@"
    check_goblin_http || exit 1

    TOKEN=$(generate_token)

    echo_info "Starting Goblin Inspector (HTTP mode)"
    echo_info "===================================="
    echo ""
    echo -e "  Goblin:    ${GREEN}$GOBLIN_URL${NC}"
    echo -e "  Token:     ${YELLOW}$TOKEN${NC}"
    echo -e "  UI:        ${BLUE}http://localhost:$INSPECTOR_PORT${NC}"
    echo ""

    # For HTTP transport, we use the Inspector's HTTP mode
    # The Inspector acts as an MCP client connecting to Goblin
    CMD="npx @modelcontextprotocol/inspector"
    CMD="$CMD --transport http"
    CMD="$CMD --serverUrl $GOBLIN_URL"
